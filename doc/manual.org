...
When a processor is loaded, it will provide the implementation with a
list of it's processed variable. And each Processed Variable will
specify it's *Input*. Each item in the input is known as a *Input
Argument*. Each Input Argument will have a name and description. Each
Processed Variable must specify a finite list of Input Arguments. When
the Processed Variable is defined, it will only be provided the input
argument it had specified. If the user had not provided an argument, a
0string ("") will be used.
...
*** TODO - refacting using "Input Argument" Static Values
